...
def apply_l2_norm(keys, epsilon=1e-12):
    """
    对注意力权重中的键进行L2归一化。

    参数:
    - keys: 注意力权重的键，形状为 (batch_size, num_heads, sequence_length, key_dim)
    - epsilon: 防止除以零的小值

    返回:
    - normalized_keys: 归一化后的键
    """
    norm = torch.norm(keys, dim=-1, keepdim=True)
    normalized_keys = keys / (norm + epsilon)
...
class RNAAttention(nn.Module):
    def __init__(self, config: RNAConfig):
        ...
        self.bpp_att = nn.Sequential(nn.Conv2d(4,self.num_heads//2,3,padding=1),
                                        nn.BatchNorm2d(self.num_heads//2),
                                        nn.ReLU(inplace=True),
                                        nn.Conv2d(self.num_heads//2,self.num_heads,1),)
        self.bpp_rev = nn.Sequential(nn.ConvTranspose2d(self.num_heads,4,3,padding=1),
                                        nn.BatchNorm2d(4),
                                        nn.Sigmoid()
                                        )
    ...
    def forward(
        ...
        logn: Optional[torch.Tensor] = None,
        ...
        # key_states = F.normalize(key_states,-1)
        value_states = value_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)

        cos, sin = self.rotary_emb(value_states, seq_len=q_len)
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids)
        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3))  / self.scale
        attn_weights *=  logn.unsqueeze(1).unsqueeze(2).unsqueeze(3)
        
        if attention_mask is not None:
            attn_weights = attn_weights + attention_mask
        bpp_weight = None
        if bpps is not None:
            bpp_weight = self.bpp_att(bpps)
            attn_weights += bpp_weight
        ...
        if bpps is not None:
            bpps = self.bpp_rev(bpp_weight) + bpps
            return attn_output, bpps
        else:
            return attn_output,bpps
...
class RNADecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        bbps: Optional[torch.Tensor] = None,
        logn: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        padding_mask: Optional[torch.Tensor] = None,
    ) ->Tuple[torch.Tensor,Optional[torch.Tensor]]:
        ...
        hidden_states,bbps = self.self_attn(
            hidden_states,
            attention_mask,
            bbps,
            logn,
