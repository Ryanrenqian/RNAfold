...
class RNAAttention(nn.Module):
    ...
    def forward(
        ...
        bsz, q_len, d_len = hidden_states.size()
        ...
        scale_factor = torch.log2(attention_mask.sum((1,2)))/9 
        cos, sin = self.rotary_emb(value_states, seq_len=q_len)
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids)

        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim) * scale_factor # log n缩放attention
