...
class RNA_Dataset(Dataset):
    def __init__(self, df, mode='train', seed=2023, fold=0, nfolds=4, 
        ...
        if mode == 'train':
             m = ((df_2A3['reads'].values > 100) & (df_2A3['signal_to_noise'].values > 0.75))| ((df_DMS['reads'].values > 100) & (df_DMS['signal_to_noise'].values > 0.75))
        else:
...
class RNA_Dataset_Test(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {'seq':torch.from_numpy(seq), 'mask':mask,'origin_seq':self.df.loc[idx]['sequence']}, \
               {'ids':ids}
...
test_file = '../datas/test_sequences.parquet'
train_file = '../datas/train_data.parquet'

def load_data(stage,df=None,bs=256,num_workers=8,device='cuda'):
    if stage =='train':
        if df is None:
            df = pd.read_parquet(train_file)
        Dataset = RNA_Dataset
        ds_train = Dataset(df, mode='train', fold=0, nfolds=4)
        ds_train_len = Dataset(df, mode='train', fold=0, 
                    nfolds=4, mask_only=True)
        sampler_train = torch.utils.data.RandomSampler(ds_train_len)
        len_sampler_train = LenMatchBatchSampler(sampler_train, batch_size=bs,
                    drop_last=True)
        train_dl = DeviceDataLoader(DataLoader(ds_train, 
                    num_workers=num_workers,
                    persistent_workers=False,batch_sampler=len_sampler_train))
        return train_dl,None
    if stage =='eval':
        df = pd.read_parquet(train_file)  
        ds_train = RNA_Dataset(df, mode='eval', fold=0, nfolds=4)
        ds_train_len = RNA_Dataset(df, mode='eval', fold=0, 
                    nfolds=4, mask_only=True)
        sampler_train = torch.utils.data.RandomSampler(ds_train_len)
        len_sampler_train = LenMatchBatchSampler(sampler_train, batch_size=bs,
                    drop_last=True)
    ...
    if stage == 'label':
        ...
        len_sampler_train = LenMatchBatchSampler(sampler_train, batch_size=bs,
                    drop_last=True)
        label_dl = DeviceDataLoader(DataLoader(ds_train, 
                    num_workers=num_workers,
                    persistent_workers=False,batch_sampler=len_sampler_train))
        df_test = pd.read_parquet(test_file)    
        ds = RNA_Dataset_Test(df_test)
        unlable_dl = torch.utils.data.DataLoader(ds, batch_size=bs, 
                    shuffle=False, drop_last=False,persistent_workers=False, num_workers=num_workers)
        return label_dl, unlable_dl
    return None,None