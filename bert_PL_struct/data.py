...
class RNA_Dataset(Dataset):
    def __init__(self, df, mode='train', seed=2023, fold=0, nfolds=4, 
        ...
        self.struct = df_2A3['structure'].values
    ...
    def __getitem__(self, idx):
        ...
        struct = [self.struct_map[s] for s in struct]
        seq = np.array(seq)
        struct = np.array(struct)
        mask = torch.zeros(self.Lmax, dtype=torch.bool)
        mask[:len(seq)] = True
        seq = np.pad(seq,(0,self.Lmax-len(seq)))
        struct = np.pad(struct,(0,self.Lmax-len(struct)))
        ...
        return {'seq':torch.from_numpy(seq), 'struct':struct,'mask':mask,'origin_seq':self.seq[idx]}, \
               {'react':react, 
                # 'react_err':react_err,
                # 'sn':sn, 
                'mask':mask}
...
test_file = '../datas/data2/test_sequences.parquet'
train_file = '../datas/data2/train_data_small.parquet'

def load_data(stage,df=None,bs=256,num_workers=16,device='cuda'):
    if stage =='train':
        if df is None:
    ...
    if stage == 'label':
        ...
        unbale_dl = DeviceDataLoader(torch.utils.data.DataLoader(ds, batch_size=bs, 
                    shuffle=False, drop_last=False, num_workers=num_workers), device)
        return label_dl, unbale_dl
    return None,None